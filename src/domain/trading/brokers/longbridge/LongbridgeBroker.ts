...
export class LongbridgeBroker implements IBroker {
  ...
  async searchContracts(pattern: string): Promise<ContractDescription[]> {
    ...
    for (const entry of STATIC_CONTRACT_REGISTRY) {
      if (
        entry.symbol.toUpperCase()   === needle ||
        entry.lbSymbol.toUpperCase() === needle ||
        entry.stockName.toUpperCase().includes(needle)
      ) {
    ...
    if (results.length === 0) {
      for (const entry of STATIC_CONTRACT_REGISTRY) {
        if (
          entry.symbol.toUpperCase().startsWith(needle) ||
          entry.stockName.toUpperCase().includes(needle)
        ) {
          const desc = new ContractDescription()
          desc.contract = makeContract(entry.lbSymbol)
  ...
  async getAccount(): Promise<AccountInfo> {
    try {
      ...
      let totalCash    = new LbDecimal(0)
      let totalNet     = new LbDecimal(0)
      let baseCurrency = 'HKD'

      for (const bal of rawBalances) {
        totalCash    = totalCash.add(bal.totalCash)
        totalNet     = totalNet.add(bal.netAssets)
        baseCurrency = bal.currency || baseCurrency
      }
      ...
      return {
        ...
        unrealizedPnL:  '0',
        buyingPower:     primary ? lbToString(primary.buyPower) : lbToString(totalCash),
      }
    } catch (err) {
  ...
  async getPositions(): Promise<Position[]> {
    try {
      ...
      // Fetch live quotes for all positions in one API call
      ...
      if (symbols.length > 0) {
        try {
          ...
          // If quote fails, livePrices stays empty — will use costPrice as fallback
        }
      }

      for (const channel of channels) {
        for (const pos of channel.positions ?? []) {
          positions.push(this.mapStockPosition(pos, livePrices))
        }
      }
      return positions
    } catch (err) {
  ...
  private mapStockPosition(pos: StockPosition, livePrices: Map<string, number>): Position {
    ...
    // makeContract() now embeds Chinese name in symbol (e.g. "腾讯控股 (700.HK)")
    ...
    return {
      ...
      marketValue:  qty.mul(mktPrice).abs().toString(),
      unrealizedPnL: mktPrice.sub(cost).mul(qty).toString(),
      realizedPnL:  '0',
    }
  }
